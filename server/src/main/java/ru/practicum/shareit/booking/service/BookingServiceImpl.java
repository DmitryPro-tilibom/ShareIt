...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto getBookingById(long bookingId, long userId) {
        ...
        if (booker.getId() != userId && owner.getId() != userId) {
            throw new IllegalViewAndUpdateException("Только автор или владелец может просматривать данное бронирование");
        }
        return BookingMapper.toBookingDto(booking);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BookingDto> getAllByBooker(Integer from, Integer size, String state, long bookerId) {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BookingDto> getAllByOwner(Integer from, Integer size, String state, long ownerId) {
        BookingState bookingState;
        try {
            bookingState = BookingState.valueOf(state);
        } catch (IllegalArgumentException e) {
